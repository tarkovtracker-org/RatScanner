...
Credential validation in `ChangeConnectionDialog` and `SettingsTracking` is operation-owned: dismissal detaches and cancels pending sources, while each async operation disposes its own source only after unwinding. Check cancellation on the component dispatcher after each awaited validation, persistence, and activation before starting the next step or publishing success. The persistence helper's off-dispatcher cancellation check cannot cover dismissal while the component continuation is queued, so after a successful `PersistCandidateAsync` the component calls `CredentialPersistenceCoordinator.RestoreIfCanceledAsync` (roll the committed candidate back, then throw) rather than a bare `ThrowIfCancellationRequested`; tracker activation can return normally after canceled progress retrieval. Deterministic component-handler lifecycle tests run on a Blazor dispatcher without rendering or live tracker credentials; the hosted WebView smoke remains the rendering check.
